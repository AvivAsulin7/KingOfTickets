...
python -m pip install -q -r %PROJECT_NAME%/code/backend/requirements.txt
...
python %PROJECT_NAME%\code\backend\hotix_server\manage.py collectstatic --no-input --clear
xcopy %PROJECT_NAME%\code\backend\hotix_server\static output\%PROJECT_NAME%\static\static\ /eqv
